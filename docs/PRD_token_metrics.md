...
- [x] Bar, line (break-even), and waterfall charts generated by the notebook from metrics.json. **DONE** — `scripts/gen_charts.py` renders `docs/assets/break_even_line.png` (`break_even_line`, T_build y-intercept + break-even crossing) embedded as Figure 6 in `notebooks/archlens_analysis.ipynb` and in README §Token economics.
