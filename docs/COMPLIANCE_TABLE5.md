...
| 3 | Test coverage >= 85% (statement + branch) | PASS | `uv run pytest --cov=archlens --cov-branch` -> 936 passed, 1 skipped, 96.77% |
...
| 11 | Graphify pipeline + Obsidian vault (hot/index/wiki/log) | PASS | `deliverables/graphify-out/`; `deliverables/bugsinpy-vault/`; supplemental `obsidian/` |
...
| 13 | Token before/after with >= 70% savings (or amortization) | PASS (historical ledger) | 97.08% persisted billed development ledger (`metrics/out/token_metrics.json`); supplemental debug token study in `metrics/out/debug_token_study.json` |
| 14 | 4 knowledge-quality metrics measured before/after | PASS (inconclusive proxy) | BugsInPy top-3 proxy regenerated without fabricated scores; see `metrics/out/knowledge_quality_metrics.json` |
...
All Table-5 rows are covered, with target-repo evidence scoped after the BugsInPy retarget and
partner merge.
