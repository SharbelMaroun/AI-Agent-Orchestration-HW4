...
- **FR-29** — After EVERY change, QAAgent SHALL run the unit test suite; no fix is ever kept on red. **Shipped:** on the `--loop` `LoopController` path the `TestGate` runs `uv run pytest` each iteration and a red verdict fails SC-4 so the fix is rejected (never merged off its iteration branch — branch-isolation undo); the history-preserving `git revert` rollback helper is implemented and unit-tested (`tests/test_rollback.py`, `tests/test_red_gate_rollback.py`) for committed iterations rather than auto-invoked in the loop transition. See `docs/PRD_improvement_loop.md` §7 "Implementation status".
