...
| FR-AO-08 | QAAgent SHALL run after EVERY RefactorAgent change; no fix is kept on a red QA verdict. **Shipped:** the `--loop` `LoopController` enforces this by branch-isolation + SC-4 rejection (a red `TestGate` verdict fails SC-4, so the fix is never merged off its `archlens/iter-<N>-<fix-id>` branch); the `git revert` rollback helper is implemented and unit-tested (`tests/test_red_gate_rollback.py`) for committed iterations. (The `loop` subcommand's multi-agent runner uses a dependency-free AST-parse QA gate; the real `uv run pytest` gate is on the `--loop` path — see README "Two loop entry points".) |
